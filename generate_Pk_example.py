...
k_knots = [k_min,1.e-3,k_max]

ln_A_knots = [0.,0.2,0.]

f = interp1d(np.log10(k_knots),ln_A_knots)
...
while ks[-1] <= float(k_max) :
    ...
    # as written, the max ks this introduces is above k_max, but only after 10ish digits,
    #but its enought for an out of interpolation error
